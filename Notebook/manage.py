from flask import send_from_directory,Flask, render_template, flash, redirect, url_for, session, request, logging
...
def index():
    ...
    count = 3 # 每页显示数量
    page = request.args.get('page') # 获取当前页码4
    ...
    if 'username' in session:
        ...
        user = '游客'
    return render_template('home.html', articles=articles,page=int(page),user=user) # 渲染模板
...
def login():
    ...
    if form.validate_on_submit():   # 如果提交表单，并字段验证通过
        ...
        if password_candidate==password: # 调用verify方法验证，如果为真，验证通过
            # 写入session
...
def register():
    ...
    if request.method == 'GET':
        return render_template('register.html',form=form)
         ...
         # 实例化表单类
        if form.validate():  # 如果提交表单，并字段验证通过
        # 从表单中获取字段
            username = request.form['username']
            password = request.form['password']
            email = request.form['email']
            sql = "INSERT INTO users (username,password,email) VALUES ('%s','%s','%s')" % (username,password,email)  # 根据用户名查找user表中记录
            db = MysqlUtil()  # 实例化数据库操作类
            db.insert(sql)  # 获取一条记录
